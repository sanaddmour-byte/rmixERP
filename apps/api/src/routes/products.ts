...
function toApi(row: ProductRow): Product {
  return {
    id: row.id,
    ...
    name: row.name,
    code: row.code,
    characteristicStrengthMpa: row.characteristicStrengthMpa,
    unit: row.unit,
...
async function findActive(tx: Tx, id: string): Promise<ProductRow | undefined> {
  const [row] = await tx
    .select()
    .from(product)
    .where(and(eq(product.id, id), isNull(product.voidedAt)));
  return row;
}

productsRouter.get("/products", requireAuth, requirePermission(MODULE, "view"), async (req, res) => {
  ...
  if (queryString(req, "format") === "csv") {
    sendCsv(res, "products.csv", items.map(toApi), [
      { key: "id", header: "ID" },
...
productsRouter.get("/products/:id", requireAuth, requirePermission(MODULE, "view"), async (req, res) => {
...
productsRouter.post("/products", requireAuth, requirePermission(MODULE, "create"), async (req, res) => {
  ...
  const created = await withTenant(db, req.auth!.companyId, async (tx) => {
    const [row] = await tx
      ...
      .values({
        ...
        name: input.name,
        code: input.code,
        characteristicStrengthMpa: input.characteristicStrengthMpa ?? null,
        unit: input.unit ?? "m3",
...
productsRouter.put("/products/:id", requireAuth, requirePermission(MODULE, "edit"), async (req, res) => {
  ...
  const updated = await withTenant(db, req.auth!.companyId, async (tx) => {
    ...
    const [row] = await tx
      ...
      .set({
        ...
        ...(input.characteristicStrengthMpa !== undefined && {
          characteristicStrengthMpa: input.characteristicStrengthMpa,
        }),
...
productsRouter.delete("/products/:id", requireAuth, requirePermission(MODULE, "void"), async (req, res) => {
  const parsed = VoidProductBody.safeParse(req.body ?? {});
  ...
  const voided = await withTenant(db, req.auth!.companyId, async (tx) => {
    ...
    const [row] = await tx.update(product).set({ voidedAt: new Date() }).where(eq(product.id, id)).returning();
