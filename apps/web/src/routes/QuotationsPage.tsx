...
function QuotationDetail({ quotationId, onConverted }: { quotationId: string; onConverted: () => void }) {
  const detail = useGetQuotation(quotationId);
  const products = useListProducts({ page: 1, pageSize: 100 });
  const chargeTypes = useListChargeTypes({ page: 1, pageSize: 100 });

  const send = useSendQuotation(refetchOnSuccess(detail));
  const accept = useAcceptQuotation(refetchOnSuccess(detail));
  const reject = useRejectQuotation(refetchOnSuccess(detail));
  const expire = useExpireQuotation(refetchOnSuccess(detail));
  const convert = useConvertQuotation();
  const createLine = useCreateQuotationLine(refetchOnSuccess(detail));
  const voidLine = useVoidQuotationLine(refetchOnSuccess(detail));
  const createCharge = useCreateQuotationLineCharge(refetchOnSuccess(detail));
  const voidCharge = useVoidQuotationLineCharge(refetchOnSuccess(detail));
  ...
  if (!q) return <p className="text-navy-500">Not found.</p>;

  function handleAddLine(e: React.FormEvent) {
    e.preventDefault();
    createLine.mutate({ quotationId, data: { productId, quantityM3 } });
    setProductId("");
    setQuantityM3("");
  }
  ...
  function handleConvert() {
    ...
    convert.mutate(
      { id: quotationId },
      {
        onSuccess: (result) => {
          setConverting(false);
          if (result.status === 201) onConverted();
        },
        onError: () => setConverting(false),
      },
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          Quotation — <span className="capitalize">{q.status}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {q.status === "draft" && (
            <Button size="sm" disabled={send.isPending} onClick={() => send.mutate({ id: quotationId })}>
              Send
            </Button>
          )}
          {q.status === "sent" && (
            <Button size="sm" disabled={accept.isPending} onClick={() => accept.mutate({ id: quotationId })}>
              Accept
            </Button>
          )}
          {q.status === "sent" && (
            <Button
              ...
              onClick={() => reject.mutate({ id: quotationId, data: {} })}
            >
              Reject
            </Button>
          )}
          {q.status === "sent" && (
            <Button
              ...
              onClick={() => expire.mutate({ id: quotationId })}
            >
              Mark Expired
            </Button>
          )}
          {q.status === "accepted" && (
            <Button variant="accent" size="sm" disabled={converting} onClick={handleConvert}>
                    ...
                    {q.status === "draft" && (
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => voidLine.mutate({ quotationId, id: line.id, data: {} })}
                        >
                          Remove
                        </Button>
                      </div>
                    ...
                    <td className="py-1 text-end">
                      {q.status === "draft" && (
                        <Button
                          ...
                          onClick={() => voidCharge.mutate({ quotationId, lineId: line.id, id: charge.id, data: {} })}
                        >
                          Remove
                        </Button>
                      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-navy-200 text-left text-navy-500 dark:border-navy-800 dark:text-navy-400">
                ...
                <th className="py-2 pe-4 font-medium">Valid Until</th>
              </tr>
            </thead>
            <tbody>
              {list.isLoading && (
